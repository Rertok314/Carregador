...
// Máquina de estados
enum Estado { DISPONIVEL, DIGITANDO_CPF, DIGITANDO_TELEFONE, MOSTRAR_SALDO, FINALIZADO, CARREGANDO };
...
void mostrarSaldo() {
  int botaoSim = digitalRead(botao_verde);
  int botaoNao = digitalRead(botao_vermelho);
  lcd.clear();
  lcd.noBacklight();
  delay(500);
  lcd.backlight();
  ...
  while (true) {
    botaoSim = digitalRead(botao_verde);
    botaoNao = digitalRead(botao_vermelho);
...
void digitarCampo(String &campo, String nomeCampo) {
  if (tecla >= '0' && tecla <= '9') {
    ...
    if (campo.length() > 0) campo.remove(campo.length() - 1);
    mostrarDado(nomeCampo, campo);
  }
  else if (tecla == '#') { 
    campo = "";
    ...
    if (nomeCampo == "CPF") {
      if (campo.length() != 11) {
        ...
        cpfConfirmado = true;
        lcd.clear(); lcd.print("CPF cadastrado!"); lcd.setCursor(0, 1); lcd.print(campo);
        delay(1500);
